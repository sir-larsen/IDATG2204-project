...
|/company/customer_rep/orders{:employee_nr}/{:order_number}/{:new_state} | Changes a state of an order from the current one to the new specified in the url. | Test that the order gets assigned its new state and that only legal states goes through | Alexander Eckhoff Larsen | While developing the specified endpoint|
